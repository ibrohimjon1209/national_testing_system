...
const App = () => {
  ...
  if (!isTelegramWebApp) {
    return (
      <div className="flex flex-col h-screen justify-center items-center bg-[#242f34] text-white text-center p-10">
        <h1 className="text-2xl font-bold mb-4">Bu sayt faqat Telegram Web App uchun</h1>
        <p className="mb-6">Iltimos, botdan foydalaning</p>
  ...
  return (
    <div className="flex flex-col h-auto justify-between items-center overflow-hidden overflow-y-auto">
      ...
      <div className="fixed w-full mt-[90vh] ">
        {location.pathname !== "/sign_in" && location.pathname !== "/404" && location.pathname !== "/register" && (
          <Navbar />
        )}
      </div>
    </div>
